...
struct UserInfo {
  ...
  uint32_t tag_count;   // Array size - bounds checking essential
};
...
public:
  ...
  static Message *deserialize(const uint8_t *data,
                              size_t length); // Returns nullptr on error
...
private:
  ...
  static size_t read_string(const uint8_t *buffer, size_t remaining,
                            ProtocolString &str);
};
